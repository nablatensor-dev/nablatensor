 ...
 * operations. The kernel bodies are the CUDA backend's, which HIPRTC accepts as
 * source-compatible.
 *
...
public final class RocmBackend implements ComputeBackend {
  ...
  private static final String[] KERNELS = {
      ...
      "random_uniform", "random_normal", "reduce_sum", "reduce_max", "sum_axis0",
      "reduce_axis_sum", "reduce_axis_max", "reduce_axis_argmax", "reduce_axis_max_backward",
      "broadcast_to", "conv2d_fwd", "conv2d_dx", "conv2d_dw", "relu_backward"
  };
  ...
  private synchronized void ensureInitialized() {
    ...
    byte[] code = HipRuntime.compile(KERNEL_SOURCE, deviceInfo.arch());
  ...
  public DeviceBuffer matmul(DeviceBuffer a, DeviceBuffer b) {
    ...
    for (long offset = 0; offset < totalTiles; offset += Integer.MAX_VALUE) {
      ...
      HipRuntime.launchBlocks2d(functions.get("matmul_tiled"), blocks, 16, 16,
  ...
  public DeviceBuffer batchedMatmul(DeviceBuffer a, DeviceBuffer b) {
    ...
    for (long offset = 0; offset < totalTiles; offset += Integer.MAX_VALUE) {
      int blocks = Math.toIntExact(Math.min(Integer.MAX_VALUE, totalTiles - offset));
      HipRuntime.launchBlocks2d(functions.get("batched_matmul_tiled"), blocks, 16, 16,
  ...
  public DeviceBuffer fused(Expr expr, DeviceBuffer[] inputs) {
    ...
    long function = fusedFunctions.computeIfAbsent(source,
        src -> HipRuntime.loadFunction(HipRuntime.compile(src, deviceInfo.arch()), "fused_kernel"));
  ...
  private static String generateFusedSource(Expr expr, int numInputs) {
    ...
    StringBuilder source = new StringBuilder(
        "extern \"C\" __global__ void fused_kernel(float* out");
   ...
   * name), post-order, so a diamond in the expression tree is computed once.
   */
