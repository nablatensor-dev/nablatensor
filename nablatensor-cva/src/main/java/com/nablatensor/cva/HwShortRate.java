 ...
 * coefficient is an {@link ADouble} input, so one adjoint sweep of a valuation
...
public final class HwShortRate {
  ...
  private final ADouble meanReversion;
  ...
  public State step(State s, ADouble z) {
  ...
  public ADouble discountFactor(State s) {
  ...
  public ADouble bond(ADouble rt, double t, double horizon) {
    ...
    ADouble b = meanReversion.mul(-dtau).exp().neg().add(1.0).div(meanReversion);
    ADouble term = sigma.mul(sigma).div(meanReversion.mul(4.0))
        .mul(b).mul(b)
