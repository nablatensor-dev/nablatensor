...
| `cuda` | fp32 | 3.8×10⁷ † | **4.4 min** † | fused forward+adjoint CUDA kernel, NVRTC-compiled |

† The dev box has no NVIDIA GPU. The `cuda` row is the **same `AsianRiskRun`
harness on a Colab Tesla T4** (`notebooks/gpu-bench.ipynb`) — a datacenter GPU,
not the 780M iGPU the other GPU rows use, so compare it to `vulkan`/`rocm` only
in kind. On that T4 the same harness also puts `opencl` at 1.9×10⁷ fp32
(~9 min) and `cuda` fp64 at 4.6×10⁶ (~36 min); the fp32 price reconciles to the
fp64 oracle to `~1×10⁻⁵`.
