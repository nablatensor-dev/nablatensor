...
- **Phase 2** — ⬤ *slice done*: aggregation layer (`nablatensor-risk`), scenario DSL (`nablatensor-scenario`), **FRTB SA-SBM equity** (delta + vega + curvature, 3 scenarios), **ISDA SIMM equity** (delta + vega + concentration). ⬡ *remaining*: XVA, VaR / ES, AVA, SA-CCR, IRRBB, other risk classes, multi-GPU.
