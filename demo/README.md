...
Three **real** jshell sessions aimed at a trading / quant audience. Nothing is
...
`greeks-on-gpu.sh` and `adjoint-vs-bump.sh` pick the fastest **fp32** adjoint
engine this machine has — `vulkan`, else `rocm`, else `cpu-jit` — and say which
in a dim line under the title. `calibrate-a-smile.sh` is a host-side optimiser
over an analytic formula, so it runs on `cpu-jit` regardless. Override the GPU
pick with `NABLATENSOR_DEMO_ENGINE=rocm` (or `vulkan` / `cpu-jit`).
...
Any pty recorder works — for example [`asciinema`](https://asciinema.org):

```bash
COLUMNS=100 LINES=32 asciinema rec greeks.cast -c './demo/greeks-on-gpu.sh'
agg greeks.cast greeks.gif      # optional: GIF for a slide
```
