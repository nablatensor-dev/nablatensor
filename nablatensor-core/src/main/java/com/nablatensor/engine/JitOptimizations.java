 ...
 *
 * <p>The default is {@link LevelEnum#NONE}: the generated kernel is a straight-line
 ...
 * {@linkplain #level(LevelEnum) picking a level} or by
 * {@linkplain #of(CategoryEnum...) naming individual categories}.
 *
 * <pre>{@code
 * Nabla.model(base, this::asian)
 *     .greeks().fp64().threads(8).on("cpu-jit")
 *     .jit(JitOptimizations.LevelEnum.LOW_RISK)          // level form
 *     .build();
 *
 * Nabla.model(base, this::asian)
 *     .greeks().fp64().threads(8).on("cpu-jit")
 *     .jit(JitOptimizations.CategoryEnum.ROLLED_LOOPS,   // category form
 *          JitOptimizations.CategoryEnum.DRAW_CACHE)
 *     .build();
 * }</pre>
 *
...
public final class JitOptimizations {
  ...
  public enum CategoryEnum {
     ...
     * single non-repeated run. Enabled only by {@link LevelEnum#ALL} or by naming
     * this category.
     */
    DRAW_CACHE,
     ...
     * only by {@link LevelEnum#ALL} or by naming this category.
     */
    FAST_MATH
  }

  /** A named bundle of {@link CategoryEnum} values. */
  public enum LevelEnum {
     ...
     * carry no memory or accuracy trade-off: {@link CategoryEnum#ROLLED_LOOPS}.
     */
    LOW_RISK,

    /**
     * Every optimization, including ones that cost memory
     * ({@link CategoryEnum#DRAW_CACHE}) or trade accuracy and are not always faster
     * ({@link CategoryEnum#FAST_MATH}).
     */
    ALL
  }

  /** The default: no optimizations. */
  public static final JitOptimizations NONE = new JitOptimizations(EnumSet.noneOf(CategoryEnum.class));

  private final Set<CategoryEnum> enabled;

  private JitOptimizations(Set<CategoryEnum> enabled) {
    this.enabled = enabled;
  }

  /** The optimizations bundled under {@code level}. */
  public static JitOptimizations level(LevelEnum level) {
    return switch (Objects.requireNonNull(level, "level")) {
      case NONE -> NONE;
      case LOW_RISK -> of(CategoryEnum.ROLLED_LOOPS);
      case ALL -> of(CategoryEnum.values());
    };
  }

  /** Exactly the named categories, and nothing else. */
  public static JitOptimizations of(CategoryEnum... categories) {
    EnumSet<CategoryEnum> set = EnumSet.noneOf(CategoryEnum.class);
  ...
  public JitOptimizations with(CategoryEnum category) {
    EnumSet<CategoryEnum> set = EnumSet.noneOf(CategoryEnum.class);
    set.addAll(enabled);
    set.add(Objects.requireNonNull(category, "category"));
  ...
  public boolean has(CategoryEnum category) {
