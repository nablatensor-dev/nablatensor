...
import com.nablatensor.engine.wgpu.WgpuAadKernel;
...
/**
 * The wgpu kernel's checkpointed adjoint sweep and its multi-market dispatch
 * recompute or share forward values instead of keeping them; both must return
 * exactly the bits of the plain kernel, not merely agree to a tolerance. The
 * WebGPU counterpart of {@link VulkanCheckpointParityTest}. Skips when no
 * WebGPU device is present.
 */
class WgpuCheckpointParityTest {

  private static final String CKPT = "nablatensor.wgpu.ckpt";

  @BeforeAll
  static void requireWgpu() {
    assumeTrue(WgpuAadKernel.wgpuAvailable(), "no WebGPU device on this machine");
  }
  ...
  private static AadResult replay(AadTape tape, AadOptions options, double spot) {
    try (WgpuAadKernel kernel = WgpuAadKernel.compile(tape, options)) {
